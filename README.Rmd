...
write("Imports: lattice",
      file = "meupacote/DESCRIPTION", append = TRUE)
cat(system("cat meupacote/DESCRIPTION", intern = TRUE), sep = "\n")
```
...
cat(system("Rscript -e 'check(\"meupacote/\")'", intern = TRUE),
    sep = "\n")
```
...
## Para mostrar o output da chamada da funcao foi necessario chamar o
## Rscript dentro do system
cat(system("Rscript -e 'build(\"meupacote/\")'", intern = TRUE),
    sep = "\n")
```
...
```{r echo=FALSE, comment=NA}
cat(system("head -2 .travis.yml", intern = TRUE), sep = "\n")
```
